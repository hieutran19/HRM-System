...
export default function SelectAPI(props: any) {
  const {
    labels,
    id,
    required,
  ...
  const fetchOptions = async (newPage = 1) => {
    ...
    try {
      ...
      if (fetchedOptions?.length > 0) {
        setOptions((prevOptions) => [...(newPage === 1 ? [] : prevOptions), ...fetchedOptions]);
        setHasMore(newPage < res?.data?.last_page);

        if (newPage === 1 && setDefaultValue && !value && fetchedOptions.length > 0) {
      ...
      <div className={styles.select}>
        <Select
          ...
          options={[
            ...
            ...(defaultValue &&
            !options.some((option) => formatOption(option).value == formatOption(defaultValue).value) &&
            defaultValue != null
